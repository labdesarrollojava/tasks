...
public class TaskResource {
	 ...
	 *         task, or with status 400 (Bad Request) if the task has already an ID
	...
	public ResponseEntity<Task> createTask(@Valid @RequestBody Task task) throws URISyntaxException {
		...
		if (!SecurityUtils.isCurrentUserInRole(AuthoritiesConstants.ADMIN)) {
			task.setUser(SecurityUtils.getCurrentUserLogin().get());
		}
	 ...
	 *         task, or with status 400 (Bad Request) if the task is not valid, or
	 *         with status 500 (Internal Server Error) if the task couldn't be
	 *         updated
	 * @throws URISyntaxException
	 *             if the Location URI syntax is incorrect
	 */
	...
	public ResponseEntity<Task> updateTask(@Valid @RequestBody Task task) throws URISyntaxException {
		...
		Task result = taskRepository.save(task);
		return ResponseEntity.ok().headers(HeaderUtil.createEntityUpdateAlert(ENTITY_NAME, task.getId().toString()))
				.body(result);
	}
	...
	public ResponseEntity<List<Task>> getAllTasks(Pageable pageable) {
		...
		if (SecurityUtils.isCurrentUserInRole(AuthoritiesConstants.ADMIN)) {
			...
			page = this.taskRepository.findAllByUser(SecurityUtils.getCurrentUserLogin().get(), pageable);
		}
	 ...
	 * @return the ResponseEntity with status 200 (OK) and with body the task, or
